...
const StationDiagram = ({ buildProgress = 1, firing = 0, showLabels = true, variant = 'DS1', size = 520 }) => {
  ...
  return (
    <svg viewBox={`0 0 ${size} ${size}`} className="w-full h-full">
      ...
      {stage(0.15) && (
        <g opacity={Math.min((buildProgress - 0.15) * 8, 1)} stroke={THEME.amber} fill="none" strokeWidth="0.5" opacity="0.5">
          {[...Array(11)].map((_, i) => {
            const lat = (i + 1) * 15 - 90;
      ...
      {stage(0.75) && (
        <g opacity={Math.min((buildProgress - 0.75) * 4, 1)}>
          {[...Array(120)].map((_, i) => {
            ...
            const r = sphereR * (0.5 + Math.random() * 0.35);
...
// Small gauge for radial stats
const RadialGauge = ({ value, max = 100, label, unit = '', color = THEME.amber, size = 120 }) => {
  ...
  return (
    <div style={{ width: size }} className="flex flex-col items-center">
      <svg viewBox={`0 0 ${size} ${size}`} width={size} height={size}>
        ...
        />
        <text x={size/2} y={size/2 + 3} textAnchor="middle" fontSize="14" fontFamily="'JetBrains Mono', monospace" fill={THEME.text} fontWeight="bold">
...
const StructureTab = () => {
  ...
  const materialTotal = MASS_BUDGET.reduce((a, b) => a + b.mass, 0);
