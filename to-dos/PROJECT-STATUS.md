...
**Current phase:** Phase 4 peer review complete — all 5 sprints (S4.0–S4.5) closed. Audit findings: numerical cross-check 0 discrepancies, all four budgets close; citation forward 0 broken refs; HW↔DR bidirectional table clean; 4 broken cross-refs and 1 dead reverse-reference identified and remediated. Phase 5 publication is the next active phase.
...
4. ~~Phase 4 peer review — S4.0 through S4.5~~ — done (M-04, D-15, 2026-04-26).
5. **Phase 5 publication** — now the highest-priority item. S5.1 typesetting-path setup is partly done (Typst pipeline operational since Phase 3); S5.2 front matter (cover, approval block, revision history table, TOC, list-of-figures, list-of-tables, acronym short-list, distribution / classification note); S5.3 PDF build v2 — install JetBrains Mono + Chakra Petch system-wide and iterate `typeset/template.typ` to compress page count from 111 → 30–60; S5.4 release (proof-read, figure spot-check, equation spot-check, SHA-256, archive source + manifest, tag release). Two reader-pass nice-to-haves should be addressed during S5.2 polish: clarify §12.3 DS-2 3-min-recharge requirement vs aspiration; clarify §12.4 Endor SLD-26 single-point design vs DR-11 retrofit doctrine.
6. **S3.5 illustrative-figure iteration (parallel)** — re-run Nano Banana 2 prompts for figures with label artifacts: F-2.1 (cm/m unit error in inset standoff label), F-A.1 / F-7.4 (prompt hex codes leaked as visible text in data blocks). Once iterated, flip remaining 13 §D.3 status rows DRAFT → FINAL and close M-05. Independent of Phase 5; can run in user time.
