...
**Energy arithmetic.** A capital-ship kill needs ~10¹⁵–10¹⁸ J (vaporize 10⁶–10⁸ kg at ~10¹⁰ J/kg coupling). A 3-min recharge at capital-ship power is 5.5 × 10¹²–5.5 × 10¹⁵ W — 10⁹–10¹² × below planet-kill power draw. **Capital-ship shots are consistent with DS-1-class reactor output at low duty cycle.** The reactor scale-up is only mandatory if DS-2 maintains planet-kill capability at 3-minute cadence, which canon implies but the films never demonstrate.

**Spec stance (resolution of S4.5 reader-pass query).** This specification adopts the **conservative reading**: the 3-minute "few-minute" cadence is a *capital-ship recharge requirement*, not a *planet-kill recharge requirement*. The DS-2 reactor scale-up to 3 cores is therefore primarily redundancy (one core offline, station operational) and capacitor-bank/optics bandwidth rather than a 10⁹–10¹² × power-density jump. A full-power planet-kill at 3-minute cadence is treated as **canonical aspiration**, not an adopted design requirement; full-power recharge in the spec remains 24 hours, matching DS-1. This reading keeps the per-core hypermatter physics consistent across DS-1 and DS-2 and lets the canon multi-core architecture serve its real engineering purposes (redundancy, partial-power agility) without compounding HW-2 / HW-3 / HW-4 concessions. Confidence: medium — alternative reading (full planet-kill at 3 min) is canonically defensible but extends concession depth without observational support.
...
**Spec stance (resolution of S4.5 reader-pass query).** The as-deployed DS-2 SLD-26 architecture **violates DR-11** ("Shield generator MUST have ≥ 3 independent redundant projection nodes with voting logic"). The spec treats this not as a deliberate design trade but as a recurrence of the DS-1 single-point-of-failure pattern (`09-vulnerability-analysis.md` §9.4) — the redundancy discipline that the unified fix list calls for was abandoned for political/tactical reasons (the trap at Endor required a deployable shield-projection asset on the inhabited moon). The recommended remediation in this spec is therefore a **post-Endor retrofit**: 3+ independent generator nodes, geographically or astronomically separated, with voting logic and graceful degradation. The doctrinal lesson — that the Empire could engineer redundancy but consistently chose monolithic single-node systems for terror-doctrine signalling — is preserved in §9.4.
